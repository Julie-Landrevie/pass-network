...
def plot_pass_network(nodes, edges, team_name, period_label, title_suffix=''):
    ...
    if nodes.empty or edges.empty:
        ax.text(60, 40, 'Données insuffisantes', color='white',
    ...
    for _, node in nodes.iterrows():
        ...
        ax.scatter(node['avg_x'], node['avg_y'],
                   s=size, c=ACCENT, zorder=5, alpha=0.9,
                   linewidths=2, edgecolors='white')

        # Short name label
        short = node['player'].split()[-1] if len(node['player'].split()) > 1 else node['player']
        ax.text(node['avg_x'], node['avg_y'] - 3.5, short,
                color='white', fontsize=7.5, fontweight='bold',
                ha='center', va='top', zorder=6,
                fontfamily='monospace')
        ax.text(node['avg_x'], node['avg_y'], str(int(node['pass_count'])),
                color=DARK_BG, fontsize=7, fontweight='bold',
                ha='center', va='center', zorder=7)

    # Title
    ax.set_title(f'{team_name}  |  Pass Network  |  {period_label}{title_suffix}',
                 color='white', fontsize=13, fontweight='bold',
                 pad=14, fontfamily='monospace')

    # Legend
    ax.text(1, 80, '● node size = passes made', color='#718096',
            fontsize=7.5, fontfamily='monospace')
    ax.text(1, 77, '→ edge width = pass frequency', color='#718096',
            fontsize=7.5, fontfamily='monospace')
...
with st.sidebar:
    ...
    <div style='padding: 16px 0 20px 0;'>
        ...
        <div style='font-family: Space Mono, monospace; font-size: 10px; color: #4fd1c5; 
    ...
    st.markdown("---")
    min_passes = st.slider("Min. passes par liaison", min_value=1, max_value=10, value=3)

    st.markdown("---")
    st.markdown("""
    <div style='font-family: Space Mono, monospace; font-size: 10px; color: #4a5568; line-height: 1.8;'>
...
<div style='display: flex; align-items: center; justify-content: space-between; 
    ...
    <div style='text-align: right; font-family: Syne, sans-serif;'>
        ...
        <div style='font-family: Space Mono, monospace; font-size: 11px; color: #4a5568; margin-top: 6px;'>
            {period_filter}
        </div>
    </div>
</div>
...
st.markdown('<div class="section-header">📊 Métriques de Structure</div>', unsafe_allow_html=True)
...
with col_t1:
    st.markdown(f"<div style='font-family:Space Mono,monospace;font-size:12px;color:#4fd1c5;text-align:center;font-weight:700;margin-bottom:12px;'>{team1}</div>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown(metric_card('Total Passes', metrics1.get('total_passes', '—')), unsafe_allow_html=True)
    with c2: st.markdown(metric_card('Connexions', metrics1.get('connections', '—')), unsafe_allow_html=True)
    ...
    with c5: st.markdown(metric_card('Centralisation', f"{metrics1.get('centralization', 0):.1f}%"), unsafe_allow_html=True)
    with c6: st.markdown(metric_card('Passes/Lien', f"{metrics1.get('avg_link_passes', 0):.1f}"), unsafe_allow_html=True)
...
with col_t2:
    st.markdown(f"<div style='font-family:Space Mono,monospace;font-size:12px;color:#f6ad55;text-align:center;font-weight:700;margin-bottom:12px;'>{team2}</div>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown(metric_card('Total Passes', metrics2.get('total_passes', '—')).replace('#4fd1c5', '#f6ad55'), unsafe_allow_html=True)
    with c2: st.markdown(metric_card('Connexions', metrics2.get('connections', '—')).replace('#4fd1c5', '#f6ad55'), unsafe_allow_html=True)
    ...
    with c5: st.markdown(metric_card('Centralisation', f"{metrics2.get('centralization', 0):.1f}%").replace('#4fd1c5', '#f6ad55'), unsafe_allow_html=True)
    with c6: st.markdown(metric_card('Passes/Lien', f"{metrics2.get('avg_link_passes', 0):.1f}").replace('#4fd1c5', '#f6ad55'), unsafe_allow_html=True)


# ─── TABS ─────────────────────────────────────────────────────────────────────

tab1, tab2, tab3, tab4 = st.tabs([
    "🕸️ Pass Networks", "🌡️ Heatmaps", "📈 Centralité", "⚔️ Comparaison"
])


# TAB 1 — PASS NETWORKS
with tab1:
    st.markdown('<div class="section-header">🕸️ Réseaux de passes — Passes complétées uniquement</div>', unsafe_allow_html=True)
    ...
    # Top passeurs
    st.markdown('<div class="section-header">🔝 Joueurs clés</div>', unsafe_allow_html=True)
    colp1, colp2 = st.columns(2)

    def top_players_html(nodes, accent):
        if nodes.empty:
            return "<div style='color:#718096'>Données insuffisantes</div>"
        top = nodes.sort_values('pass_count', ascending=False).head(5)
        html = ""
        for _, r in top.iterrows():
            short = r['player'].split()[-1]
...
# TAB 2 — HEATMAPS
with tab2:
    st.markdown('<div class="section-header">🌡️ Heatmaps de positions de passe</div>', unsafe_allow_html=True)
    col_a, col_b = st.columns(2)
    with col_a:
    ...
    st.info("💡 **Lecture** : Les zones chaudes indiquent où l'équipe initie ses passes. Une concentration haute = jeu en profondeur ; concentration médiane = build-up prudent.")


# TAB 3 — CENTRALITY
with tab3:
    st.markdown('<div class="section-header">📈 Analyse de centralité — Betweenness</div>', unsafe_allow_html=True)
    col_a, col_b = st.columns(2)
    with col_a:
    ...
    st.markdown('<div class="section-header">📋 Détail des métriques joueurs</div>', unsafe_allow_html=True)
    ...
    def player_metrics_table(nodes, team_name, accent):
        ...
        display.columns = ['Joueur', 'Passes', 'Betweenness', 'Passes Reçues', 'Passes Émises']
        display = display.sort_values('Passes', ascending=False)
        display['Betweenness'] = display['Betweenness'].round(3)
        display['Passes Reçues'] = display['Passes Reçues'].round(0).astype(int)
        display['Passes Émises'] = display['Passes Émises'].round(0).astype(int)
    ...
    st.info("💡 **Betweenness centrality** : mesure combien de fois un joueur est sur le chemin le plus court entre deux autres joueurs. Score élevé = pivot tactique clé du jeu de passes.")


# TAB 4 — COMPARISON
with tab4:
    st.markdown('<div class="section-header">⚔️ Comparaison des structures d\'équipe</div>', unsafe_allow_html=True)

    fig7 = plot_comparison(metrics1, metrics2, team1, team2)
    st.pyplot(fig7, use_container_width=True)
    plt.close()

    st.markdown('<div class="section-header">📖 Interprétation tactique</div>', unsafe_allow_html=True)
    ...
    if abs(d1 - d2) > 0.05:
        ...
        insights.append(f"**Densité** : {more} présente un réseau de passes plus dense, suggérant plus d'options et de connexions entre joueurs.")

    c1, c2 = metrics1.get('clustering', 0), metrics2.get('clustering', 0)
    if abs(c1 - c2) > 0.05:
        more = team1 if c1 > c2 else team2
        insights.append(f"**Clustering** : {more} tend à jouer en triangles/clusters serrés — jeu de possession structuré.")
    ...
    if abs(ce1 - ce2) > 5:
        ...
        insights.append(f"**Centralisation** : {more} dépend davantage d'un ou deux relayeurs clés, tandis que {less} distribue le jeu plus équitablement.")

    ap1, ap2 = metrics1.get('avg_link_passes', 0), metrics2.get('avg_link_passes', 0)
    if abs(ap1 - ap2) > 2:
        more = team1 if ap1 > ap2 else team2
        insights.append(f"**Liaisons** : {more} utilise ses connexions de façon plus intensive (plus de passes par paire de joueurs).")
    ...
    for insight in insights:
        ...
        <div style='background:#111827;border:1px solid #2d3748;border-left:3px solid #4fd1c5;
                    ...
                    font-family:Syne,sans-serif;font-size:14px;color:#cbd5e0;line-height:1.6;'>
            {insight}
        </div>
        """, unsafe_allow_html=True)

    # Full metrics table comparison
    st.markdown('<div class="section-header">📊 Tableau comparatif complet</div>', unsafe_allow_html=True)
    comp_df = pd.DataFrame({
        'Métrique': ['Total Passes', 'Connexions actives', 'Density', 'Avg Clustering', 'Centralisation (%)', 'Passes moy./liaison'],
        team1: [
